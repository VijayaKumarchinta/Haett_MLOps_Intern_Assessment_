...
active_d = merged[merged["is_active"] == True]["duration_days"]
inactive_d = merged[merged["is_active"] == False]["duration_days"]

print(f"\n{HASH}")
print("  [2] STEP 1: users.is_active DRIVES subscription duration")
print(SEP)
print(f"  Active   users (is_active=True) : mean={active_d.mean():7.0f}d, median={active_d.median():7.0f}d")
print(f"  Inactive users (is_active=False): mean={inactive_d.mean():7.0f}d, median={inactive_d.median():7.0f}d")
...
ct = pd.crosstab(
    ...
    merged["ends_after_snapshot"].map({True: "ends_after_snap", False: "ends_before_snap"}),
    margins=True,
)
print(f"\n{HASH}")
...
ends_in_window = (merged["end_date"] >= SNAPSHOT_DATE) & (merged["end_date"] <= window_end)

print(f"\n{HASH}")
print("  [4] STEP 3: Subscription end dates relative to snapshot")
print(SEP)
print(f"  Already ended BEFORE snapshot:       {ends_before.sum():5d} users ({ends_before.mean()*100:.1f}%)")
print(f"  Ends WITHIN 30-day window (CHURN):   {ends_in_window.sum():5d} users ({ends_in_window.mean()*100:.1f}%)")
print(f"  Ends AFTER 30-day window:            {(~ends_before & ~ends_in_window).sum():5d} users ({(~ends_before & ~ends_in_window).mean()*100:.1f}%)")

# -- 5. The leakage proof --
merged["is_sub_active"] = merged["status"] == "active"
cancelled = merged[merged["is_sub_active"] == False]
active_sub = merged[merged["is_sub_active"] == True]

print(f"\n{HASH}")
print("  [5] PROOF OF LEAKAGE: is_sub_active vs churn")
print(SEP)
print(f"  Users with is_sub_active=False (cancelled): {len(cancelled)}")
print(f"    Churned: {cancelled['churned'].sum():.0f}, churn rate: {cancelled['churned'].mean()*100:.1f}%")
print(f"    --> ~0% because subscription already ended (can't end twice)")

print(f"\n  Users with is_sub_active=True (active): {len(active_sub)}")
print(f"    Churned: {active_sub['churned'].sum():.0f}, churn rate: {active_sub['churned'].mean()*100:.1f}%")
print(f"    --> TRUE at-risk rate for active subscribers")

# -- 5b. Cancel reason cross-check --
print(f"\n  Cross-check: cancel_reason dummies (residual leakage?)")
for reason in ["too_expensive", "delivery_issues", "diet_change", "traveling", "not_enough_variety"]:
    ...
    if len(has_reason) > 0:
        ...
        print(f"    Users with any cancel_reason: "
...
for _, row in corr_df.head(15).iterrows():
    print(f"  {row['feature']:<35} {row['correlation']:>+10.4f}  {row['abs_corr']:>8.4f}")

# -- 7. Feature group comparison --
categories = {
    "Subscription (tenure)": ["subscription_tenure_days", "monthly_price", "n_plan_changes"],
    "Cancel reasons (leakage residue)": [c for c in features.columns if c.startswith("cancel_reason_")],
    "Recency": ["days_since_last_order", "days_since_high_value", "days_since_last_late"],
    "Frequency": ["total_orders", "unique_meal_plans", "order_frequency_per_month", "weekend_order_ratio", "mean_days_between_orders"],
    "Monetary": ["total_spent", "avg_order_value", "max_order_value", "min_order_value", "avg_rating", "spending_trend", "value_variability"],
    "Engagement": ["avg_app_logins", "avg_recipes_viewed", "avg_meals_skipped", "total_support_tickets", "total_referral_clicks", "login_decline", "recipe_decline", "avg_orders_per_week", "recent_avg_logins", "recent_avg_recipes"],
    "Demographic": ["age", "age_group_code", "diet_balanced", "diet_keto", "diet_low_carb", "diet_mediterranean", "diet_paleo", "diet_vegan"],
}
...
with open(Path(__file__).resolve().parent.parent / "models" / "leakage_analysis.txt", "w") as f:
    ...
    f.write(f"Top 5 feature correlations:\n")
